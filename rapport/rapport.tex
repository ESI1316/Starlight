...
5.4}\footnote{\label{va}voir Annexe~\ref{ref}}, pour l'accès à la bibliothèque
graphique, et compilé à l'aide de
\textbf{g++}\textsuperscript{\ref{va}} dans sa version 4.8 (et
compatible 4.9). La norme \textbf{ISO/IEC
14882:2011}\textsuperscript{\ref{va}}
...
\textbf{Makefile}\textsuperscript{\ref{va}} qui s'occupe de
\begin{itemize}
 ...
 */
 T variable;
\end{lstlisting}
et cette documention\textsuperscript{\ref{va}} a est uniquement présente dans les fichiers \textbf{headers}.
La documentation, au format \LaTeX et html, a été généré à l'aide de l'outil
Doxygen\textsuperscript{\ref{va}} dans sa dernière version
...
U.M.L.\textsuperscript{\ref{va}} a été généré 
...
\subsection{html}
Les documents ``HTML'' disponible dans \url{ressources/other} sont des textes
formatées à l'aide de ``CSS''. Ils sont utilisés lors de l'affichage des
...
\textbf{camelCase}\textsuperscript{\ref{va}}
\begin{lstlisting}[frame=single,language=C++]
T uneBonneVariable;
...
cartésien\textsuperscript{\ref{va}} : 
\begin{itemize}
...
\begin{enumerate}
	\item \textbf{les sons} sont disponibles lors 
		\begin{itemize}
		...
		\end{itemize}
	\item \textbf{les couleurs} sont disponibles en fonction de la longueur d'onde du
		rayon\textsuperscript{\ref{va}}
	\item \textbf{l'icone du logiciel} disponible et a été désigné par
		\textbf{Kriwin Paul} sur un support libre d'image d'étoile.
\end{enumerate}
\chapter{Conclusion}
...
\begin{itemize}
	\item L'ensemble des sons disponibles dans \path{ressources/sounds}
		proviennent de {Sound bible} \url{http://soundbible.com} sous la license 
	    {Attribution 3.0 License} \url{co://creativecommons.org/licenses/by/3.0/},
	\item l'icone a été dessiné par \textbf{Kriwin Paul} sous
		\href{http://www.gimp.org/}{Gimp},
	\item calcul de la longueur d'onde :
		\url{http://www.physics.sfasu.edu/astro/color/spectra.html},
	\item Catch Framework \url{https://github.com/philsquared/Catch} consulté
		\today,
	\item {U.M.L. - Unified Modeling Language } \url{http://www.uml.org/} consulté
		\today,
	\item {Doxygen} \url{http://doxygen.org/} consulté \today,
	\item Plan cartésien \url{http://www.cslaval.qc.ca/sitsatlll/maths2003/cartesien.html}
		consulté \today,
	\item Principe du CamelCase \url{https://fr.wikipedia.org/wiki/CamelCase}
		consulté \today,
	\item Qt Creator disponible dans sa dernière version sur \url{http://www.qt.io/developers/}
		consulté le \today,
	\item Catalogue des normes ISO \url{http://www.iso.org/iso/iso_catalogue/catalogue_tc/catalogue_detail.htm?csnumber=50372}
		consulté le \today,
	\item GNU GCC \url{https://gcc.gnu.org/} consulté le \today,
	\item GNU MakeFile \url{https://www.gnu.org/software/make/manual/html_node/Makefiles.html}
		consulté le \today, 
\end{itemize}
